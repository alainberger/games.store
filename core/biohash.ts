/*
 * Cancellable biometrics transformation.
 * Input: embedding (512 floats) + deterministic user seed from KMS/keystore.
 * Output: normalized transformed embedding (biohash) ready for storage/comparison.
 */

const projectionCache: Map<string, Float32Array[]> = new Map();
...
function generateProjection(seed: string, dimension: number): Float32Array[] {
  const cacheKey = `${seed}:${dimension}`;
  const cached = projectionCache.get(cacheKey);
  if (cached) return cached;

  const rng = createRng(seed);
  const matrix: Float64Array[] = Array.from({ length: dimension }, () => new Float64Array(dimension));
...
export function applyBiohash(embedding: number[] | Float32Array, userSeed: string): Float32Array {
